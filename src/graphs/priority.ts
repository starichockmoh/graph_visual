...
class Heap {
   ...
   * @return {boolean}
   */
  hasLeftChild(parentIndex: number | undefined) {
    return (
      this.getLeftChildIndex(parentIndex as any) < this.heapContainer.length
    );
  }
   ...
   * @return {*}
   */
  parent(childIndex: number | undefined) {
    return this.heapContainer[this.getParentIndex(childIndex as any)];
  }
  ...
  remove(item: any, comparator = this.compare) {
    ...
    for (let iteration = 0; iteration < numberOfItemsToRemove; iteration += 1) {
      ...
      if (indexToRemove === this.heapContainer.length - 1) {
        ...
        this.heapContainer[indexToRemove as any] = this.heapContainer.pop();
        ...
        if (
          ...
          (!parentItem ||
            this.pairIsInCorrectOrder(
              parentItem,
              this.heapContainer[indexToRemove as any],
            ))
        ) {
...
export default class PriorityQueue extends MinHeap {
   ...
   */
  add(item: string | number, priority = 0) {
   ...
   * @return {PriorityQueue}
   */
  remove(item: any, customFindingComparator: Comparator | undefined) {
